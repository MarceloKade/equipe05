...
function Container({ children }: Props) {
  return (
    <section
      id="SaibaMais"
      className="flex flex-col justify-center py-[6.4rem]"
    >
      {children}
    </section>
  );
...
function ContainerQuemSomos({ children }: Props) {
  return (
    <div
      id="QuemSomos"
      className=" flex flex-col justify-center xl:flex-row xl:p-[11.2rem]"
    >
      {children}
    </div>
  );
...
function ContainerText({ children }: Props) {
  return <div className="max-w-[527px] xl:py-0 xl:px-16 mr-16">{children}</div>;
}

function ContainerImage({ children }: Props) {
  return <div className="mt-[2.0rem]">{children}</div>;
}
