...
function ContainerSection({ children }: Props) {
  return (
    <section
      id="Comunidade"
      className="w-full flex justify-center xl:px-[11.95rem] xl:py-[7.2rem]"
    >
      {children}
    </section>
  );
...
function Container({ children }: Props) {
  return (
    <div className="relative w-full xl:w-[122.5rem] flex justify-center items-center">
...
function TextContainer({ children }: Props) {
  return (
    <div className="flex flex-col gap-[1.5rem] items-center">{children}</div>
  );
}
...
function ShapeContainer({ children }: Props) {
  return (
    <div className="absolute flex justify-center items-center w-full h-full">
      <div className="flex justify-between w-2/3 h-1/2">{children}</div>
    </div>
  );
}
